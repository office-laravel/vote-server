...
<option value="{{ $lang->id }}" @if($question->lang_id== $lang->id) @selected(true) @endif>{{ $lang->name }}</option>
@endforeach
                    ...
                    <div class="form-group row">
                        <div class="col-sm-10">
                            ...
                            <select name="category_id" id="category_id" class="form-controll">
                                ...
                                <option value="{{ $cat->id }}" @if($question->category_id== $cat->id) @selected(true) @endif >{{ $cat->title }}</option>
                                @endforeach
                                                        </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-controll" name="content" id="content"
                                placeholder="* نص السؤال" value="{{ $question->content }}">
                    ...
                    @php
                    $content="";$check=0;
                 if( $question->answers->where('sequence',$i)->first()){
                    $content=$question->answers->where('sequence',$i)->first()->content;
                    $check=$question->answers->where('sequence',$i)->first()->is_correct;
                 }
                @endphp            
                        <div class="col-sm-6 col-12 ans-col">
                            <div class="form-group row">
                                ...
                                <div class="col-sm-11 col-10">
                                    ...
                                    placeholder="* الاجابة {{ $i }}" value="{{$content}}">
                                    <span id="answer_content-{{ $i }}-error" class="error invalid-feedback"></span>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-4 col-12 ans-col">
                            <input type="radio" id="is_correct_{{ $i }}" name="is_correct" value="{{$i}}" @if ($check==1)@checked(true)@endif>
                        </div>
